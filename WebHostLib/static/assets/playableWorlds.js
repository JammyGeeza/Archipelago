...
window.addEventListener('load', () => {
  ...
  const toggleButtons = document.querySelectorAll('details[data-game]');
  ...
  gameSearch.addEventListener('input', (evt) => {
    ...
    toggleButtons.forEach((header) => {
      ...
      if (header.getAttribute('data-game').toLowerCase().includes(evt.target.value.toLowerCase())) {
        ...
        // If this game sits inside a stability-group wrapper, open
        // that too - a closed ancestor <details> hides its children
        // regardless of the child's own open state. No-op on pages
        // with no such wrapper (e.g. the main supported-games page).
        const group = header.closest('details.stability-group');
        if (group) {
          group.setAttribute('open', '1');
        }
      } else {
...
// A "sort scope" is whatever container games should be sorted and
// re-appended WITHIN. If the page has stability-group wrappers
// (Stable/Unstable/Broken on Main), each one is its own independent
// scope - games never leave their group. If there are none (the main
// supported-games page), #games itself is the one and only scope,
// exactly matching the original, pre-stability-grouping behavior.
function getSortScopes() {
  const groups = document.querySelectorAll('#games > details.stability-group');
  return groups.length > 0 ? Array.from(groups) : [document.getElementById('games')];
}

// The actual game <details> that are DIRECT children of a given
// scope - deliberately not a deep querySelectorAll, so a scope never
// picks up games belonging to some other nested scope.
function getScopedGames(scope) {
  return Array.from(scope.children).filter((el) => el.matches('details[data-game]'));
}
...
// POST_CHECK_TIER/getEffectiveTier above. Runs independently within
// each sort scope (see getSortScopes) - on a page with stability
// groups, a game never leaves its own group; on the main page,
// there's just the one scope (#games), so this is unchanged.
function sortGames() {
  getSortScopes().forEach((scope) => {
    const details = getScopedGames(scope);
    ...
    details.forEach((el) => scope.appendChild(el));
  });
}
...
// Same per-scope behavior as sortGames() - stays within each
// stability group where present.
