...
#region 'The Grand Scheme' Quests
    ...
    set_rule(world.get_location("Find the Catacombs", player),
             lambda state: state.sl_has_all_packs(["Humble Beginnings", "Explorers"], player) or # <- Player has Explorers pack for chance to find Catacombs
    ...
    set_rule(world.get_location("Kill the Demon", player), # <- Will be an 'Event' location if Goal is 'Kill the Demon' or a Location Check if Goal is 'Kill the Demon Lord'
                    lambda state: state.can_reach_location("Build a Farm", player) and # <- Player can make a Farm
                                  state.can_reach_location("Cook Raw Meat", player) and # <- Player can make an Omelette
                                  state.can_reach_location("Reach Moon 24", player) and # <- Player has all equipment and Shed
...
#region 'Strengthen Up' Quests
    ...
    set_rule(world.get_location("Build a Smithy", player),
             lambda state: state.sl_has_idea("Smithy", player) and # <- Player has Ideas for Smithy
                        ...
                        #    (state.sl_has_all_ideas(["Brick", "Plank"], player) or state.sl_has_pack("Logic and Reason", player)) and # <- Player can access Bricks / Planks
                           state.sl_has_pack("Humble Beginnings", player))
    ...
    set_rule(world.get_location("Train a Wizard", player),
             lambda state: state.sl_has_idea("Magic Wand", player) and # <- Player has Idea for Magic Wand
                           state.sl_has_any_ideas(["Slingshot", "Spear"], player) and # <- Player can fight enemies from Strange Portal that can drop Magic Dust
    ...
    set_rule(world.get_location("Have a Villager with Combat Level 20", player),
             lambda state: state.sl_has_any_ideas(["Slingshot", "Spear"], player) and # <- Player has at least one basic weapon
                           state.sl_has_pack("Humble Beginnings", player))
...
#region 'Discovery' Quests
    ...
    set_rule(world.get_location("Buy something from a Travelling Cart", player),
             lambda state: state.can_reach_location("Train a Wizard", player)) # <-  Cart always spawns on Moon 19 or 10% chance every odd moon from Moon 9, so needs to be able to fight Portal enemies
...
#region 'Ways and Means' Quests
    ...
    set_rule(world.get_location("Get an Iron Bar", player),
             lambda state: state.sl_has_all_ideas(["Iron Bar", "Smelter"], player) and # <- Player has Idea for Smelter and Iron Bar
                           (state.sl_has_all_ideas(["Brick", "Plank"], player) or state.sl_has_pack("Logic and Reason", player)) and # <- Player has access to Bricks / Planks 
                           state.sl_has_pack("Humble Beginnings", player))
...
#region 'Construction' Quests
    ...
    set_rule(world.get_location("Sell a Card at a Market", player),
             lambda state: state.sl_has_idea("Market", player) and # <- Player has Idea for Market
                           (state.sl_has_all_ideas(["Brick", "Plank"], player) or state.sl_has_pack("Logic and Reason", player)) and # <- Player can get bricks and planks
                           state.sl_has_pack("Humble Beginnings", player)) # <- Player has packs for additional resources
...
# TODO: Maybe these can be used to ensure that the order of 'progressive' ideas is correct (and all these checks are set to EXCLUDED to prevent them containing progression items)
# and then these can be what Kill the Demon point to as reference for success. Worth a try?

    set_rule(world.get_location("Reach Moon 6", player),
             lambda state: state.sl_has_idea("Brick", player) and # <- Trialling to insert useful ideas early
                           state.sl_has_pack("Humble Beginnings", player))

    set_rule(world.get_location("Reach Moon 12", player),
             lambda state: state.sl_has_any_ideas(["Slingshot", "Spear"], player) and # <- Player has Idea for at least one basic weapons
                           state.sl_has_all_ideas(["Plank", "Wooden Shield"], player) and # <- Player has Idea for Wooden Shirld
                        #    (state.sl_has_all_ideas(["Plank"], player) or state.sl_has_pack("Logic and Reason", player)) and # <- Player has access to Planks (for Wooden Shield)
                           state.can_reach_location("Make a Stick from Wood", player) and # <- Player can make Stick (for basic weapons)
    ...
    set_rule(world.get_location("Reach Moon 24", player),
             lambda state: state.sl_has_any_ideas(["Boomerang", "Sword", "Throwing Stars"], player) and # <- Player has Idea for at least one advanced weapon
                           state.sl_has_idea("Iron Shield", player) and # <- Player has Idea for Iron Shield
                           state.can_reach_location("Build a Smithy", player) and # <- Player can build Smithy (for advanced weapons / shield)
...
#region 'The Dark Forest' Quests
    ...
    set_rule(world.get_location("Complete the first wave", player),
             lambda state: state.can_reach_location("Find the Dark Forest", player)) # <- Player can reach The Dark Forest
    ...
    set_rule(world.get_location("Build a Stable Portal", player),
             lambda state: state.sl_has_idea("Stable Portal", player) and # <- Player has Idea for Stable Portal
                           (state.sl_has_idea("Brick", player) or state.sl_has_pack("Logic and Reason", player)) and # <- Player has access to Bricks
